## @hishprorg/necessitatibus-minus-quos &mdash; object viewer &nbsp; &nbsp; [![npm](https://img.shields.io/npm/v/@hishprorg/necessitatibus-minus-quos.svg)](https://www.npmjs.com/package/@hishprorg/necessitatibus-minus-quos) [![build status](https://github.com/hishprorg/necessitatibus-minus-quos/actions/workflows/ci.yml/badge.svg?branch=main)](https://github.com/hishprorg/necessitatibus-minus-quos/actions/workflows/ci.yml)

**@hishprorg/necessitatibus-minus-quos** &mdash; pronounced *vúed* or *vue dd* &mdash;  is a Vue 3 object viewer for displaying complex JavaScript objects, Vue Reactive and Ref objects, Functions, Maps, Sets, Symbols and all JavaScript Primitives in order to monitor their changes **live** with full reactivity. Inspired by the Laravels **dd()** debugging function.
...
```js
import { VueDd } from '@hishprorg/necessitatibus-minus-quos'
...
```bash
# npm
npm install @hishprorg/necessitatibus-minus-quos
# yarn
yarn add @hishprorg/necessitatibus-minus-quos
```
...
```html
<@hishprorg/necessitatibus-minus-quos v-model="object" />
```
...
- performance optimized, even large objects like `window` can be @hishprorg/necessitatibus-minus-quos
...
| open   | `(setup) => {...}` | `{ open, pointer, level } = setup`                                             | On open by @hishprorg/necessitatibus-minus-quos itself or by a user, this emit will be called |
...
- There are many things that can be done with this package, for ex. here is what I am doing with it right now:
    ...
    - All of these features area currently in development in **XRay** component, that will soon be part of **@hishprorg/necessitatibus-minus-quos**, but feel free to develop your own versions with this package
...
Versions up to **@hishprorg/necessitatibus-minus-quos@1.1.6** supported **Vue 2.7.5** and above, but import of the package is different from Vue 3. In Vue 2 @hishprorg/necessitatibus-minus-quos is imported as default and not as a destructured `VueDd`
```js
// to install @hishprorg/necessitatibus-minus-quos with Vue 2 support
// yarn add @hishprorg/necessitatibus-minus-quos@1.1.6
// npm install @hishprorg/necessitatibus-minus-quos@1.1.6
// note: 'import VueDd...', and not 'import { VueDd }...'
import VueDd from '@hishprorg/necessitatibus-minus-quos'
```
Also note in Vue 2, you have to use `:model-value` instead of `v-model`
```html
<@hishprorg/necessitatibus-minus-quos :model-value="obj" />
```
> Note that Vue 2 version cannot support large objects like `window`. For some reason, it reaches a stack size limit and breaks. It is a known issue in Vue 2, that's why migrate to Vue 3 ASAP in order to use the full power of **@hishprorg/necessitatibus-minus-quos**
## Credits & Thanks
...
> **Highlight.js** is built into @hishprorg/necessitatibus-minus-quos to provide JavaScript function highlighting.
